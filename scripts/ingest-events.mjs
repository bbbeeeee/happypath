...
export async function fetchPermits(windowStart, windowEnd, fetchImpl = fetch) {
  const permits = [];
  for (let offset = 0; ; offset += PAGE_SIZE) {
    const url = new URL(`/resource/${DATASET_ID}.json`, NYC_API_ROOT);
    url.searchParams.set("$limit", String(PAGE_SIZE));
    url.searchParams.set("$offset", String(offset));
    url.searchParams.set("$order", "start_date_time,event_id");
    url.searchParams.set(
      "$where",
      `event_borough='Manhattan' AND end_date_time >= '${windowStart}' AND start_date_time <= '${windowEnd}'`,
    );
    ...
    if (!Array.isArray(page)) throw new Error("Permitted event response was not an array");
    permits.push(...page);
    if (page.length < PAGE_SIZE) return permits;
  }
}
