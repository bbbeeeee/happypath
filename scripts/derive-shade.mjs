...
for (const hour of HOURS) {
  ...
  for (const edge of graph.edges) {
    const from = nodeById.get(edge.from).coordinate; const to = nodeById.get(edge.to).coordinate;
    const samples = Math.max(2, Math.ceil(edge.distanceMeters / SAMPLE_METERS));
    let shaded = 0;
    for (let i = 0; i <= samples; i++) {
      const ratio = i / samples;
      if (isShaded([from[0] + (to[0] - from[0]) * ratio, from[1] + (to[1] - from[1]) * ratio], index)) shaded++;
    }
    edgeShadeByHour[edge.id][hour] = Number((shaded / (samples + 1)).toFixed(3));
  }
  console.log(`${hour}:00 — ${polygons.length} projected shadows`);
}

const output = {
  metadata: { date: DATE, hours: HOURS, latitude: LATITUDE, longitude: LONGITUDE, utcOffsetHours: UTC_OFFSET, methodVersion: "building-shadow-sampling-v1", solarMethod: "noaa-solar-approx-v1", sampleSpacingMeters: SAMPLE_METERS, sourceIds: ["nyc-building-footprints"], edgeCoverage: 1, validationStatus: "pending" },
  edgeShadeByHour,
};
...
otherSources.push({
  ...
  method_version: "building-shadow-sampling-v1",
  known_limitations: ["Uses a solar approximation pending SPA-library review", "Samples straight graph-edge chords every 8 meters", "Convex shadow hulls may overstate shadows for concave footprints", "Does not model trees, clouds, facade detail, or measured temperature"],
