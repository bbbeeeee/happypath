...
import { calendarDateForTimeZone, findOnRouteEvents, type OnRouteEvent } from "./planning/routeEvents";
...
function ensureIsometricBuildingLayer(map: MapLibreMap) {
  if (map.getLayer("isometric-buildings")) return;
  if (!map.getSource("isometric-buildings")) {
  ...
  }, map.getLayer("building-shadows") ? "building-shadows" : undefined);
}
...
export function App() {
  ...
  const [eventCalendarDay, setEventCalendarDay] = useState(() => calendarDateForTimeZone(new Date(), NYC_TIME_ZONE));
  ...
  useEffect(() => {
    const interval = window.setInterval(() => {
      setEventCalendarDay(calendarDateForTimeZone(new Date(), NYC_TIME_ZONE));
    }, 60_000);
  ...
  const onRouteEvents = useMemo(() => route
    ? findOnRouteEvents(route, {
        today: eventCalendarDay,
        departureHour: brief.departureHour,
      })
    : [], [route, brief.departureHour, eventCalendarDay]);
  ...
  useEffect(() => {
    ...
    if (isometric) ensureIsometricBuildingLayer(map);
    if (!map.getLayer("isometric-buildings")) return;
  ...
  useEffect(() => {
    ...
    if (route) {
      map.fitBounds(boundsForRoute(route), {
        ...MAP_PERSPECTIVE_CAMERA[mapPerspective],
        padding: { top: 90, right: 70, bottom: 90, left: window.innerWidth > 800 ? 460 : 70 },
        maxZoom: 16,
        duration: 650,
      });
    }
  }, [navigating, mapPerspective, mapReady, route]);
