...
describe("compareRoutes", () => {
  ...
  it("preserves curved edge geometry in either traversal direction", () => {
    const geometry = [
      [-74, 40.73],
      [-73.9996, 40.7305],
      [-73.999, 40.73],
    ...
    const forward = compareRoutes(graph, "a", "b", 15);
    const reverse = compareRoutes(graph, "b", "a", 15);
    expect(forward.fastest.coordinates).toEqual(geometry);
    expect(reverse.fastest.coordinates).toEqual([...geometry].reverse());
    expect(forward.fastest.directSunMinutes).toBe(forward.fastest.durationMinutes);
    expect(forward.fastest.greeneryPercent).toBe(0);
  });
});
