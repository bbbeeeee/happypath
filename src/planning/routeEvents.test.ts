...
import { calendarDateForTimeZone, findOnRouteEvents } from "./routeEvents";
...
describe("findOnRouteEvents", () => {
  ...
  it("counts a walk that is still on the shared block after the opening bell", () => {
    const late = findOnRouteEvents(routeThrough(["n1", "n2", "n3"], 60), { ...tripToday, departureHour: 9.5 });
    expect(late).toHaveLength(1);
  });

  it("does not surface an event after the walker has passed its block", () => {
    const earlyBlockEvent: PermittedEvent = { ...festival, segments: [{ ...festival.segments[0], nodePairs: [["n1", "n2"]], geometry: [festival.segments[0].geometry[0]] }] };
    const matches = findOnRouteEvents(routeThrough(["n1", "n2", "n3", "n4", "n5"], 120), {
      ...tripToday,
      departureHour: 9.5,
      events: [earlyBlockEvent],
    });
    expect(matches).toEqual([]);
  });

  it("surfaces an event when the walker reaches a later shared block", () => {
    const lateBlockEvent: PermittedEvent = { ...festival, segments: [{ ...festival.segments[0], nodePairs: [["n4", "n5"]], geometry: [festival.segments[0].geometry[0]] }] };
    const matches = findOnRouteEvents(routeThrough(["n1", "n2", "n3", "n4", "n5"], 120), {
      ...tripToday,
      departureHour: 9.5,
      events: [lateBlockEvent],
    });
    expect(matches).toHaveLength(1);
  });

  it("keeps a multi-day permit visible on its middle day", () => {
    const multiDayEvent = { ...festival, startsAt: "2026-08-15T10:00:00.000", endsAt: "2026-08-17T18:00:00.000" };
    expect(findOnRouteEvents(routeThrough(["n1", "n2", "n3"]), {
      ...tripToday,
...
describe("calendarDateForTimeZone", () => {
  it("uses the New York calendar day across the UTC evening rollover", () => {
    const eveningInNewYork = new Date("2026-08-17T01:30:00.000Z");
    expect(calendarDateForTimeZone(eveningInNewYork, "America/New_York")).toBe("2026-08-16");
  });
});
